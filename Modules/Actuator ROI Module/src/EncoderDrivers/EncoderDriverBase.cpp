...
void EncoderDriverBase::home(uint16_t length) { _homed_length = length; }

EncoderReading EncoderDriverBase::value() { return _cur_read; }
...
float EncoderDriverBase::velocity() {
    ...
    if (time_dif > 0) return 1000 * (_cur_read.length - _prev_read.length) / time_dif;

...
#define __stateMessage(function_name, package_name, message_type, message_val_1, val_1, message_val_2, val_2, \
                       publisher)                                                                             \
...
#define __stateMessage4Arg(function_name, package_name, message_type, message_val_1, val_1, message_val_2, val_2, \
                           message_val3, val3, message_val4, val4, publisher)                                     \
    void ODriveModule::function_name() {                                                                          \
        /* Publishes variables based on request */                                                                \
        auto message = package_name::msg::message_type();                                                         \
        message.message_val_1 = val_1;                                                                            \
...
#define __goalHandler(function_name, function_name_str, goal_type)                                                \
    rclcpp_action::GoalResponse ODriveModule::function_name(                                                      \
        const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const roi_ros::action::goal_type::Goal> goal) {      \
        this->debugLog("Received goto " function_name_str " action goal request");                                \
                                                                                                                  \
        if (!this->validateVelTorque(goal->target_joint_state.velocity[0], goal->target_joint_state.effort[0])) { \
...
#define __gotoServiceHandler(function_name, request_type, request_type_str, velocity, feed_forward, calling_function) \
...
#define __actionExecuteHandler(function_name, function_name_str, goal_type, position_condition, position_update)      \
    void ODriveModule::function_name(                                                                                 \
        ...
        while (rclcpp::ok() && (this->_velocity > 0.5 ||                 /* check if moving at a good speed */        \
            ...
            std::this_thread::sleep_for(                                                                              \
                std::chrono::milliseconds(WatchdogConstants::MAINTAIN_SLEEP_TIME)); /* Wait for feedback to be read*/ \
                                                                                                                      \
...
#define __actionAcceptHandler(function_name, function_name_str, goal_type, goto_function, execution_handler) \
    void ODriveModule::function_name(                                                                        \
        const std::shared_ptr<rclcpp_action::ServerGoalHandle<roi_ros::action::goal_type>> goalHandle) {     \
...
void ODriveModule::responseCallback(const roi_ros::msg::SerializedPacket response) {
    ...
    this->unpackVectorToArray(response.data, serializedData, __min(response.length, ROIConstants::ROI_MAX_PACKET_SIZE));
    if (!packet.importPacket(serializedData, response.length) && !moduleNodeConstants::IGNORE_MALFORMED_PACKETS) {
    ...
    if (packet.getActionCode() & ODriveConstants::MaskConstants::GETMASK) {
        ...
        switch (packet.getActionCode() & !ODriveConstants::MaskConstants::GETMASK) {
            ...
            __responseCallbackGetVariable(Current, _current, floatCast::toFloat(data, 0, 3),
                                          this->publishPowerMessage());
            __responseCallbackGetVariable(MotorTemperature, _motorTemperature, floatCast::toFloat(data, 0, 3),
                                          this->publishTemperatureMessage());
            __responseCallbackGetVariable(FETTemperature, _fetTemperature, floatCast::toFloat(data, 0, 3),
                                          this->publishTemperatureMessage());
        ...
        switch (packet.getActionCode() & !ODriveConstants::MaskConstants::GETMASK) {
            ...
            default:
                this->debugLog("Unknown action code received: " + std::to_string(packet.getActionCode()));
                break;
        }
    }
...
void ODriveModule::sendGotoPositionPacket(float position, float velocity_feedforward, float torque_feedforward) {
    ...
    if (_inputMode != ODriveConstants::TRAP_TRAJ_MODE || _inputMode != ODriveConstants::POS_FILTER_MODE) {
...
void ODriveModule::sendGotoRelativePositionPacket(float position, float velocity_feedforward,
    ...
    this->_relativeStartPosition = this->_position;  // store the relative start position (we do this here to hack it
                                                     // into the action server macro... yuck)
}
...
void ODriveModule::setMotorEnabledServiceHandler(const std_srvs::srv::SetBool::Request::SharedPtr request,
    ...
    if (_healthData._module_error) {
        response->success = false;
        response->message = "Cannot change motor state while module is in error: " + _healthData._module_error_message;
        return;
    }
    ...
    response->message = request->data ? "Motor enabled (closed-loop control)" : "Motor disabled (idle state)";
}
...
std::string ODriveModule::oDriveErrorToString(uint32_t errorCode) {
    switch (errorCode) {
        ...
        __oDriveErrorCase(ODRIVE_ERROR_CURRENT_LIMIT_VIOLATION,
        ...
        __oDriveErrorCase(ODRIVE_ERROR_DC_BUS_OVER_REGEN_CURRENT,
                          "DC bus over regen current, current limit exceeded. Auto reset attempting.");
        __oDriveErrorCase(ODRIVE_ERROR_DC_BUS_OVER_VOLTAGE,
        ...
        __oDriveErrorCase(ODRIVE_ERROR_VELOCITY_LIMIT_VIOLATION,
...
__stateMessage(publishPowerMessage, roi_ros, Power, voltage, _busVoltage, current, _current, _power_publisher_);

__stateMessage4Arg(publishStateMessage, sensor_msgs, JointState, position,
                   std::vector<double>{this->revToRad(_position)}, velocity,
                   std::vector<double>{this->revToRad(_velocity)}, name, std::vector<std::string>{"odrive.axis0"},
                   effort, std::vector<double>{_current}, _state_publisher_);

__stateMessage(publishFetTemperatureMessage, sensor_msgs, Temperature, temperature, (double)_fetTemperature, variance,
               0, _fet_temperature_publisher_);

__stateMessage(publishMotorTemperatureMessage, sensor_msgs, Temperature, temperature, (double)_motorTemperature,
               variance, 0, _motor_temperature_publisher_);
...
__gotoServiceHandler(setTorqueServiceHandler, TargetJointState, "set torque", 0, request->target_joint_state.effort[0],
...
__actionAcceptHandler(gotoPositionAcceptedHandler, "position", TargetJointState, sendGotoPositionPacket,
                      gotoPositionExecuteHandler);
...
ODriveModule::ODriveModule() : BaseModule("ODriveModule", moduleTypesConstants::O_DRIVE) {
    ...
    this->_power_publisher_ = this->create_publisher<roi_ros::msg::Power>("roi_ros/odrv/axis0/power", 10);
    this->_state_publisher_ = this->create_publisher<sensor_msgs::msg::JointState>("roi_ros/odrv/axis0/state", 10);
    this->_fet_temperature_publisher_ =
        this->create_publisher<sensor_msgs::msg::Temperature>("roi_ros/odrv/axis0/fet_temperature", 10);
    this->_motor_temperature_publisher_ =
        this->create_publisher<sensor_msgs::msg::Temperature>("roi_ros/odrv/axis0/motor_temperature", 10);
    ...
    this->_goto_position_service_ = this->create_service<roi_ros::srv::TargetJointState>(
        ...
        std::bind(&ODriveModule::gotoPositionServiceHandler, this, std::placeholders::_1, std::placeholders::_2));
    this->_goto_relative_position_service_ = this->create_service<roi_ros::srv::TargetJointState>(
        "roi_ros/odrv/axis0/goto_relative_position", std::bind(&ODriveModule::gotoRelativePositionServiceHandler, this,
                                                               std::placeholders::_1, std::placeholders::_2));
    this->_set_torque_service_ = this->create_service<roi_ros::srv::TargetJointState>(
        "roi_ros/odrv/axis0/set_torque",
        std::bind(&ODriveModule::setTorqueServiceHandler, this, std::placeholders::_1, std::placeholders::_2));
    this->_set_velocity_service_ = this->create_service<roi_ros::srv::TargetJointState>(
        "roi_ros/odrv/axis0/set_velocity",
        std::bind(&ODriveModule::setVelocityServiceHandler, this, std::placeholders::_1, std::placeholders::_2));
    ...
    this->_set_motor_enabled_service_ = this->create_service<std_srvs::srv::SetBool>(
        ...
        std::bind(&ODriveModule::setMotorEnabledServiceHandler, this, std::placeholders::_1, std::placeholders::_2));

    // Initialize the ODrive specific action servers
    this->_goto_position_action_server_ = rclcpp_action::create_server<roi_ros::action::TargetJointState>(
        this->get_node_base_interface(), this->get_node_clock_interface(), this->get_node_logging_interface(),
        this->get_node_waitables_interface(), "roi_ros/odrv/axis0/goto_position",
        std::bind(&ODriveModule::gotoPositionGoalHandler, this, std::placeholders::_1, std::placeholders::_2),
    ...
    this->_goto_relative_position_action_server_ = rclcpp_action::create_server<roi_ros::action::TargetJointState>(
        this->get_node_base_interface(), this->get_node_clock_interface(), this->get_node_logging_interface(),
        this->get_node_waitables_interface(), "roi_ros/odrv/axis0/goto_relative_position",
        std::bind(&ODriveModule::gotoRelativePositionGoalHandler, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&ODriveModule::gotoRelativePositionCancelHandler, this, std::placeholders::_1),
        std::bind(&ODriveModule::gotoRelativePositionAcceptedHandler, this, std::placeholders::_1));

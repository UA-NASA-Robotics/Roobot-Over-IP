...
void Packet::setData_impSplit(uint16_t num1) {
    this->_data[0] = num1 >> 8;
    this->_data[1] = num1 & 0xFF;
}

void Packet::setData_impFloatCast(float num1) {
    floatCast::floatToUint8Array(num1, this->_data, 3, 0);
}

void Packet::setData_impFloatCast(float num1, bool endian) {
    if (endian) {
        floatCast::floatToUint8Array(num1, this->_data, 3, 0);
    } else {

...
#define __stateMessage(function_name, message_type, message_val_1, val_1, message_val_2, val_2, \
                       publisher)                                                               \
...
#define __gotoServiceHander(function_name, request_type, request_type_str, velocity, feed_forward, \
                            calling_function)                                                      \
...
#define __positionExecuteHandler(function_name, function_name_str, goal_type, position_condition, \
                                 position_update)                                                 \
    void ODriveModule::function_name(                                                             \
...
#define __positionAcceptHandler(function_name, function_name_str, goal_type, goto_function, \
                                execution_handler)                                          \
    void ODriveModule::function_name(                                                       \
        const std::shared_ptr<rclcpp_action::ServerGoalHandle<roi_ros::action::goal_type>>  \
            goalHandle) {                                                                   \
        ...
        this->goto_function(goalHandle->get_goal()->position,                               \
                            goalHandle->get_goal()->velocity_feedforward,                   \
...
void ODriveModule::responseCallback(const roi_ros::msg::SerializedPacket response) {
    ...
    if (packet.getActionCode() & ODriveConstants::MaskConstants::GETMASK) {
        ...
        __responseCallbackGetVariable(ControlMode, _controlMode, data[0],
                                      (void)"") __responseCallbackGetVariable(InputMode, _inputMode,
                                                                              data[0], (void)"")
            __responseCallbackGetVariable(
                Torque, _inputTorque, floatCast::toFloat(data, 0, 3),
                (void)"") __responseCallbackGetVariable(PositionSetPoint, _inputPosition,
                                                        floatCast::toFloat(data, 0, 3), (void)"")
                __responseCallbackGetVariable(
                    VelocitySetPoint, _inputVelocity, floatCast::toFloat(data, 0, 3),
                    (void)"") __responseCallbackGetVariable(Error,
                                                            _healthData._module_error_message,
                                                            this->oDriveErrorToString(
                                                                data[0] << 24 | data[1] << 16 |
                                                                data[2] << 8 | data[3]),
                                                            this->publishHealthMessage())
                    __responseCallbackGetVariable(Velocity, _velocity,
                                                  floatCast::toFloat(data, 0, 3),
                                                  this->publishStateMessage())
                        __responseCallbackGetVariable(Position, _position,
                                                      floatCast::toFloat(data, 0, 3),
                                                      this->publishStateMessage())
                            __responseCallbackGetVariable(BusVoltage, _busVoltage,
                                                          floatCast::toFloat(data, 0, 3),
                                                          this->publishPowerMessage())
                                __responseCallbackGetVariable(Current, _current,
                                                              floatCast::toFloat(data, 0, 3),
                                                              this->publishPowerMessage())
                                    __responseCallbackGetVariable(MotorTemperature,
                                                                  _motorTemperature,
                                                                  floatCast::toFloat(data, 0, 3),
                                                                  this->publishTemperatureMessage())
                                        __responseCallbackGetVariable(
                                            FETTemperature, _fetTemperature,
                                            floatCast::toFloat(data, 0, 3),
                                            this->publishTemperatureMessage())
...
std::string ODriveModule::oDriveErrorToString(uint32_t errorCode) {
    switch (errorCode) {
        __oDriveErrorCase(ODRIVE_ERROR_BAD_CONFIG, "Bad Configuration, invalid settings values.")
            __oDriveErrorCase(
                ODRIVE_ERROR_BRAKE_RESISTOR_DISARMED,
                "Brake resistor disarmed.") __oDriveErrorCase(ODRIVE_ERROR_CALIBRATION_ERROR,
                                                              "Calibration error, manually "
                                                              "re-calibrate motor.")
                __oDriveErrorCase(ODRIVE_ERROR_CURRENT_LIMIT_VIOLATION,
                                  "Current limit violation, current limit exceeded. Auto reset "
                                  "atemping.") __oDriveErrorCase(ODRIVE_ERROR_DC_BUS_OVER_CURRENT,
                                                                 "DC bus over current, current "
                                                                 "limit exceeded. Auto reset "
                                                                 "atemping.")
                    __oDriveErrorCase(
                        ODRIVE_ERROR_DC_BUS_OVER_REGEN_CURRENT,
                        "DC bus over regen current, current limit exceeded. Auto reset atemping.")
                        __oDriveErrorCase(
                            ODRIVE_ERROR_DC_BUS_OVER_VOLTAGE,
                            "DC bus over voltage, reduce allowed regen current. Auto reset "
                            "atemping.") __oDriveErrorCase(ODRIVE_ERROR_DC_BUS_UNDER_VOLTAGE,
                                                           "DC bus under voltage, check power "
                                                           "supply.")
                            __oDriveErrorCase(ODRIVE_ERROR_DRV_FAULT,
                                              ...
                                              "encoder "
                                              "connection"
                                              "s.") __oDriveErrorCase(ODRIVE_ERROR_ESTOP_REQUESTED,
                                                                      "E-stop requested, check for "
                                                                      "e-stop condition.")
                                __oDriveErrorCase(
                                    ODRIVE_ERROR_INITIALIZING,
                                    "Initializing, wait for initialization to complete.")
                                    __oDriveErrorCase(
                                        ODRIVE_ERROR_INVERTER_OVER_TEMP,
                                        "Inverter over temperature, reduce load or increase "
                                        "cooling.") __oDriveErrorCase(ODRIVE_ERROR_MISSING_ESTIMATE,
                                                                      "Missing estimate, check "
                                                                      "encoder connection.")
                                        __oDriveErrorCase(
                                            ODRIVE_ERROR_MISSING_INPUT,
                                            "Missing input, check network and module firmware.")
                                            __oDriveErrorCase(
                                                ODRIVE_ERROR_MOTOR_OVER_TEMP,
                                                "Motor over temperature, reduce load or increase "
                                                "cooling.") __oDriveErrorCase(ODRIVE_ERROR_NONE, "")
                                                __oDriveErrorCase(
                                                    ODRIVE_ERROR_POSITION_LIMIT_VIOLATION,
                                                    "Position limit violation, check position "
                                                    "limits.")
                                                    __oDriveErrorCase(
                                                        ODRIVE_ERROR_SPINOUT_DETECTED,
                                                        "Spinout detected, check motor and encoder "
                                                        "connections.")
                                                        __oDriveErrorCase(
                                                            ODRIVE_ERROR_SYSTEM_LEVEL,
                                                            "System level error, check ODrive "
                                                            "firmware/replace.")
                                                            __oDriveErrorCase(
                                                                ODRIVE_ERROR_THERMISTOR_DISCONNECTED,
                                                                "Motor thermistor disconnected, "
                                                                "check thermistor connection.")
                                                                __oDriveErrorCase(
                                                                    ODRIVE_ERROR_TIMING_ERROR,
                                                                    "Timing error, check motor and "
                                                                    "encoder connections.")
                                                                    __oDriveErrorCase(
                                                                        ...
                                                                        "Auto reset atemping.")
                                                                        __oDriveErrorCase(
                                                                            ODRIVE_ERROR_WATCHDOG_TIMER_EXPIRED,
                                                                            "Watchdog timer "
                                                                            "expired, check module "
                                                                            "to odrive "
                                                                            "connection.") default
...
__stateMessage(publishPowerMessage, ODrivePower, voltage, _busVoltage, current, _current,
               _power_publisher_)

    __stateMessage(publishStateMessage, ODriveState, position, _position, velocity, _velocity,
                   _state_publisher_)

        __stateMessage(publishTemperatureMessage, ODriveTemperature, motor_temp, _motorTemperature,
                       fet_temp, _fetTemperature, _temperature_publisher_)
            ...
            __gotoServiceHander(gotoPositionServiceHandler, ODriveGotoPosition, "goto position",
                ...
                __gotoServiceHander(gotoRelativePositionServiceHandler, ODriveGotoRelativePosition,
                                    "goto relative position", request->velocity_feedforward,
                                    request->torque_feedforward,
                                    this->sendGotoRelativePositionPacket(
                                        request->position, request->velocity_feedforward,
                                        request->torque_feedforward))

                    __gotoServiceHander(setTorqueServiceHandler, ODriveSetTorque, "set torque", 0,
                                        request->torque, this->sendSetTorquePacket(request->torque))

                        __gotoServiceHander(setVelocityServiceHandler, ODriveSetVelocity,
                                            "set velocity", request->velocity,
                                            request->torque_feedforward,
                                            this->sendSetVelocityPacket(
                                                request->velocity, request->torque_feedforward))

                            __goalHandler(gotoPositionGoalHandler, "position", ODriveGotoPosition)

                                __positionAcceptHandler(gotoPositionAcceptedHandler, "position",
                                                        ODriveGotoPosition, sendGotoPositionPacket,
                                                        gotoPositionExecuteHandler)

                                    __positionCancelHandle(gotoPositionCancelHandler, "position",
                                                           ODriveGotoPosition)

                                        __positionExecuteHandler(gotoPositionExecuteHandler,
                                                                 "position", ODriveGotoPosition,
                                                                 this->_position == goal->position,
                                                                 this->_position)

                                            __goalHandler(gotoRelativePositionGoalHandler,
                                                          "relative position",
                                                          ODriveGotoRelativePosition)

                                                __positionAcceptHandler(
                                                    gotoRelativePositionAcceptedHandler,
                                                    "relative position", ODriveGotoRelativePosition,
                                                    sendGotoRelativePositionPacket,
                                                    gotoRelativePositionExecuteHandler)

                                                    __positionCancelHandle(
                                                        gotoRelativePositionCancelHandler,
                                                        "relative position",
                                                        ODriveGotoRelativePosition)

                                                        __positionExecuteHandler(
                                                            gotoRelativePositionExecuteHandler,
                                                            "relative position",
                                                            ODriveGotoRelativePosition,
    ...
    : BaseModule("ODriveModule", moduleTypesConstants::O_DRIVE) {
